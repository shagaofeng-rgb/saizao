...
export function AdminDashboard() {
  ...
  const load = useCallback(async () => {
    setState("loading");
    const query = new URLSearchParams({ from: new Date(`${from}T00:00:00Z`).toISOString(), to: new Date(`${to}T23:59:59Z`).toISOString() });
  ...
  return <AdminShell><section id="main-content" className="admin-content">
      ...
      <section className="admin-filters" aria-label="数据筛选">
        ...
        <label>国家 / 地区<input value={country} onChange={(event) => setCountry(event.target.value.toUpperCase())} /></label>
        <label>来源渠道<input value={source} onChange={(event) => setSource(event.target.value)} /></label>
      ...
      {state === "setup" ? <section className="admin-callout"><h2>暂无可展示的数据</h2><p>数据将在产生有效访问或询盘后显示。</p></section> : <>
        ...
        {state === "error" ? <section className="admin-callout"><h2>暂时无法读取数据</h2><p>请稍后刷新页面重试。</p></section> : <>
          <section className="admin-grid">
            <Summary title="主要访问地区" rows={data.countries} icon={GlobeHemisphereWest} onSelect={(label) => setCountry(label)} />
            <Summary title="访问来源" rows={data.sources} icon={ChartLineUp} formatLabel={(label) => displayValue(label, "直接访问")} onSelect={(label) => setSource(label)} />
            <Summary title="热门页面" rows={data.pages} icon={Path} />
          </section>

          <section className="admin-panel admin-module-links"><div><h2>业务处理</h2><p className="admin-panel-subtitle">访客轨迹与询盘跟进已独立成页，便于连续处理工作。</p></div><div><Link className="admin-primary" href="/admin/visitors">进入访客中心</Link><Link className="admin-row-action" href="/admin/inquiries">进入询盘管理</Link></div></section>
        </>}
      </>}
    </section></AdminShell>;
}
