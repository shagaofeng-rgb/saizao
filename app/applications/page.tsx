...
export default function ApplicationsPage() { return <main id="main-content"><SiteHeader /><section className="listing-hero listing-hero--visual"><div className="listing-hero-copy"><p className="eyebrow">APPLICATIONS</p><h1>Different products.<br />A considered scent direction for each.</h1><p className="lead">Choose an application to explore the kind of development conversation we can begin together.</p></div><figure className="listing-hero-visual"><Image src="/images/hero-fragrance-still-life.jpg" alt="Fragrance ingredients and bottles in Sai Zhao's development setting" fill sizes="(max-width: 780px) 100vw, 42vw" preload className="cover-image" /></figure></section><section className="listing-grid section">{applications.map((item) => <article className="listing-card" key={item.slug}><Image src={item.image} alt={applicationImageAlt(item.title)} width={900} height={900} /><div><h2>{item.title}</h2><p>{item.description}</p><Link className="text-link" href={`/applications/${item.slug}`}>Explore {item.title} <ArrowRight size={17} /></Link></div></article>)}</section><SiteFooter /></main>; }
